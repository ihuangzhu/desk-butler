...
这是 **0.1.0 本地 release candidate（自动化候选）**，尚未正式发布，也没有配置远程 Git。本轮最终 Release 自动套件为 644 项：641 通过、3 项按显式交互/长时门禁跳过、0 失败；Release build 为 0 警告、0 错误。Windows 10/11 专用 VM 的 QQ、微信、富途全链、干净测试账户安装与卸载、真实重启、本构建 30 分钟资源测试、诊断 ZIP 用户导出和产品签名仍未过门禁。不要把当前状态理解为“全面兼容”或“已正式发布”。
